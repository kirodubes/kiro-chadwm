...
### What Changed
- Removed the `update-system` keybinding (`ctrl + alt + shift + F1` and `super + ctrl + shift + F1`). `update-system` is a personal maintenance script that had slipped into ohmychadwm and propagated across every Kiro environment; it is not part of the shipped desktop, so the binding is gone. All cheatsheet `.txt` variants were updated to match.
